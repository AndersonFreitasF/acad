...
import { professorService, CreateProfessorData } from "../services/professor";
import { Button } from "../components/ui/button";
import { Input } from "../components/ui/input";
import { Label } from "../components/ui/label";
import { Loader2, UserPlus, ArrowLeft, Save, GraduationCap } from "lucide-react";
...
export function CreateProfessor() {
  ...
  const [formData, setFormData] = useState<CreateProfessorData & { confirmSenha: string }>({
    nome: "",
    email: "",
    cpf: "",
    senha: "",
    ...
    <div className="min-h-[calc(100vh-180px)] relative overflow-hidden">
      ...
      <div className="relative z-10 py-12 px-4">
        <div className="max-w-2xl mx-auto space-y-8">
          ...
          >
            <div className="p-6 bg-neo-purple border-b-4 border-black flex items-center gap-4">
              <div className="w-14 h-14 bg-white rounded-base flex items-center justify-center border-3 border-black">
                <GraduationCap className="w-7 h-7 text-neo-purple" />
              </div>
              <div>
                <h2 className="text-xl font-heading text-white">
                  Preencha os dados do professor
                </h2>
                <p className="text-white/80 text-sm">Todos os campos são obrigatórios</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div className="flex gap-4">
                ...
                <Button
                  type="submit"
                  disabled={loading}
                  className="flex-1 h-14 text-lg font-bold bg-neo-purple text-white border-4 border-black shadow-neo hover:shadow-neo-hover hover:translate-x-[-4px] hover:translate-y-[-4px]"
                >
                  {loading ? (
